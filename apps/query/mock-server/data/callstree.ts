import {
    staticCallsTreePlaceholder,
    staticInitialLayoutPlaceholder,
    staticVersionPlaceholder,
} from '@app/components/static-hooks';
...
export function createCallsTreeData() {
    const callsTreeStub: CallsTreeData = {
        info: [
            {
                id: 'network',
                values: [1.324],
                ...
                params: [
                    {
                        id: 'common.started',
                        values: [
                            'Sat Mar 09 2024 13:40:50.013 GMT+0300 (Moscow Standard Time) (1709980850013)',
                        ],
                        type: 'String',
                        isList: false,
                        isIndex: false,
    ...
    return callsTreeStub;
}

const staticPageMock = `
<div>
<div><strong>CDT version:</strong> "${staticVersionPlaceholder}"</div>
<div><strong>Calls tree JSON:</strong> "${staticCallsTreePlaceholder}"</div>
<div><strong>Initial state JSON:</strong> "${staticInitialLayoutPlaceholder}"</div>
</div>
`;

export function createStaticCallsTree(initialState: DashboardEntity[]) {
    const callsTreeStr = JSON.stringify(createCallsTreeData());
    const initialStateStr = JSON.stringify(initialState);
    return staticPageMock
        .replace(`"${staticVersionPlaceholder}"`, "'1.0.0'")
        .replace(`"${staticCallsTreePlaceholder}"`, `'${callsTreeStr}'`)
        .replace(`"${staticInitialLayoutPlaceholder}"`, `'${initialStateStr}'`);
}
